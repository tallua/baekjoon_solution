...
public:
    constexpr segment_tree(const size_type N,
...
struct sum
{
    constexpr _Type operator() (const _Type& lhs, const _Type& rhs) const
    {
        return lhs + rhs;
    }
};

segment_tree<long long, sum<long long>> segtree(1000000, 0);
...
int main(int argc, char** argv)
{
    ...
    while(Q--)
    {
        ...
        case 1:
        {
            ...
            const auto now = segtree.get(p - 1);
            segtree.update(p - 1, x + now);
            break;
        }
        case 2:
        {
            ...
            const auto result = segtree.query_range(p - 1, q);
            cout << result << '\n';
            break;
        }
        }

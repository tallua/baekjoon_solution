...
public:
    constexpr segment_tree(size_type N,
...
int main(int argc, char** argv)
{
    ...
    segment_tree<int, sum<int>> temperatures(65536, 0);
    deque<int> input_q;
    int half_k = (K + 1) / 2;
    long long sum = 0;
    for (int n = 0; n < N; ++n)
    {
        ...
        temperatures.update(current, temperatures.get(current) + 1);

        if (input_q.size() > K)
        {
            int front = input_q.front();
            input_q.pop_front();

            temperatures.update(front, temperatures.get(front) - 1);
        }

        if (input_q.size() == K)
        {
            sum = sum + (long long)(temperatures.index_query(nth_index<int>{ half_k }));
        }
    }
